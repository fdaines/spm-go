...
const HtmlDependenciesTemplate = `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<h2>Dependencies analysis for Module: {{.Module}}</h2>
<div class="results">
<table class="analysis-summary">
    ...
    <tr>
        ...
        <th colspan="3">Dependencies Count</th>
    </tr>
    <tr>
        <th style="width:120px;">Internals</th>
        <th style="width:120px;">Externals</th>
    ...
    <tr class="{{if le .Distance 0.1}}green{{else if gt .Distance 0.6}}red{{end}}" onClick="javasript:toggleDetails('{{.Path}}-details')">
        <td class="">{{ .Name }}</td>
        <td class="">{{ .Path }}</td>
        <td class="right right-spaced">{{ .Dependencies.InternalsCount }}</td>
        <td class="right right-spaced">{{ .Dependencies.ExternalsCount }}</td>
        <td class="right right-spaced">{{ .Dependencies.StandardCount }}</td>
    </tr>
